...
def get_best_in_region_gen(match_values, coords, template_shape, num_template, angle):
    """
    Aplica supresión no máxima para eliminar coincidencias cercanas que representen la misma región.
    ...
    for i, coord in enumerate(sorted_coords):
        if mask[i]:
            ...
            # Buscar el pixel cuyo valor máximo en match_values es más parecido a angle
            # closest_pixel_index = np.argmin(np.abs(np.argmax(match_values[coord[0], coord[1]]) - angle))
            closest_pixel_index = find_best_coordinate(match_values=match_values, angle=angle)
            print("Pixel más cercano al ángulo:", closest_pixel_index)
          ...
          # Crear un diccionario con las coordenadas cercanas y sus valores en match_values y correlation
        nearby_dict = {
            tuple(coord): (match_values[coord[0], coord[1]], 
                        get_correlation(match_values, coord, num_template))
            for coord in nearby_coords
        }    
        
    
    return tuple(closest_pixel_index), nearby_dict
...
def get_correlation(match_values, coord, num_template):
    ...
    func1 = funcion_org
    func2 = funcion_comp
    # norm1 = (func1 - np.mean(func1)) / np.std(func1)
    # norm2 = (func2 - np.mean(func2)) / np.std(func2)
   
    # Calcular la correlación de Pearson
    # corr_coef, p_value = pearsonr(norm1, norm2)
    # distancia = np.linalg.norm(norm1 - norm2)
    ...
    # np_array = df.values
    # print(np_array)
    # specific_row = np_array[num_template - 1]  # Suponiendo que num_template es el índice de la fila deseada
    # print(specific_row)
   

#Para poder rotar la plantilla sin problemas de dimensiones:
#aumentar el tamaño de la imagen para que pueda hacer las rotaciones en toda la imagen 
...
def estimar_instancias_con_cluster_angular(match_values, threshold=0.25, eps_espacial=15, min_samples_espacial=5,
                                           eps_angular=0.03, min_samples_angular=3):
    """
    ...
    - eps_espacial, min_samples_espacial: DBSCAN para clustering espacial
    - eps_angular, min_samples_angular: DBSCAN para clustering angular (en el círculo)
    ...
    # Paso 3: clustering espacial
    spatial_data = np.c_[coords, angle_vec]  # (N, 4)
    spatial_labels = DBSCAN(eps=eps_espacial, min_samples=min_samples_espacial).fit_predict(spatial_data)
    ...
    for label in np.unique(spatial_labels):
        ...
        # Paso 4: clustering angular en círculo unitario
        angular_labels = DBSCAN(eps=eps_angular, min_samples=min_samples_angular).fit_predict(vectores)

        # Buscar clúster angular más numeroso
        cluster_ids, counts = np.unique(angular_labels[angular_labels != -1], return_counts=True)
        if len(cluster_ids) == 0:
            continue  # sin clúster válido

        best_cluster = cluster_ids[np.argmax(counts)]
        idx_angular = angular_labels == best_cluster

        # Paso 5: calcular ángulo medio del mejor clúster
    ...
    return resultado
def estimar_instancias_con_cluster_angular2(match_values, threshold=0.25, 
                                           distancia_maxima_espacial=4, min_samples_espacial=5,
                                           distancia_maxima_angular_grados=20, min_samples_angular=3):
    """
    Detecta instancias y agrupa los valores angulares cercanos dentro de cada instancia.
    Se queda con el grupo angular más poblado por instancia y calcula su ángulo medio.
    ...
    # Paso 3: clustering espacial
    spatial_data = np.c_[coords, angle_vec]  # (N, 4)
    spatial_labels = DBSCAN(eps=distancia_maxima_espacial, min_samples=min_samples_espacial).fit_predict(spatial_data)
    ...
    return resultado
def estimar_instancias_con_cluster_angular3(match_values, threshold=0.25, 
                                           distancia_maxima_espacial=15, min_samples_espacial=5,
                                           distancia_maxima_angular_grados=20, min_samples_angular=3):
    """
    Detecta instancias y agrupa los valores angulares cercanos dentro de cada instancia.
    Se queda con el grupo angular más poblado por instancia y calcula su ángulo medio.
    ...
    for label in np.unique(spatial_labels):
        ...
        angular_labels = clustering_angular.fit_predict(vectores)
               ...
               # Buscar clúster angular más numeroso
        cluster_ids, counts = np.unique(angular_labels, return_counts=True)

        valid = counts >= min_samples_angular
        if not np.any(valid):
            continue

        valid_cluster_ids = cluster_ids[valid]
        valid_counts = counts[valid]
        best_cluster = valid_cluster_ids[np.argmax(valid_counts)]
        idx_angular = angular_labels == best_cluster
...
def estimar_instancias_con_cluster_angular4(match_values, threshold=0.25, 
...
def match_template_con_rotaciones_gen(image, template, angle, num_template=1,num_rotations=360, threshold=0.8):
    """
    Aplica match_template con múltiples rotaciones de la plantilla y devuelve un array
    con 360 valores posibles de coincidencia por cada píxel.
    ...
    max_per_pixel = np.max(match_values, axis=2)  # Obtener el máximo valor de coincidencia para cada píxel
    max_coords = np.argwhere(max_per_pixel >= threshold)  # Coordenadas donde el máximo supera el threshold
    filtered_coord, candidates = get_best_in_region_gen(match_values, max_coords, template.shape, num_template=num_template, angle=angle)
    #filtered_coords=max_coords
    return filtered_coord, candidates
